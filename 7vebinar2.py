...
class PersonInfo:
    ...
    def path_deps(self):
        deps_list = self.department.split(" --> ")
        return " --> ".join(deps_list)

    def new_salary(self):
        letter_count = {}
        for letter in "".join(self.department.split()):
            if letter in letter_count:
                letter_count[letter] += 1
            else:
                letter_count[letter] = 1

        sorted_letters = sorted(letter_count.items(), key=lambda x: x[1], reverse=True)[:3]
        top_letters = "".join([letter[0] for letter in sorted_letters])

        salary_index = 1337 * self.age * sum([top_letters.count(letter) for letter in top_letters])
        return salary_index
